...
(deftest context-allows-subscriptions-to-call-each-other
  (let [context (context/create {:db 1} identity)
        inc-db #(inc (context/sub-val % :db))]
    (fact
      (context/sub-ctx context inc-db) => 2)))
...
(deftest subscription-results-are-memoized
  (let [*tracker (atom 0)
        ...
        f (fn fib [context n]
            ...
            (if (< n 2)
              1
              (+ (context/sub-ctx context fib (- n 2))
                 (context/sub-ctx context fib (- n 1)))))]
    (context/sub-ctx context f 10)
    (fact
      @*tracker => 11))
  (testing "a missing entry is treated like a nil entry"
    (let [context (context/create {} identity)
          f #(context/sub-val % :entry)
          _ (fact (context/sub-ctx context f) => nil)
          context (context/swap context assoc :entry 0)
          _ (fact (context/sub-ctx context f) => 0)])))
  ...
  (let [*greeting-call-counter (atom 0)
        *username-call-counter (atom 0)
        ...
        username (fn [context]
                   ...
                   (-> (context/sub-val context :db) :user :name))
        greeting (fn [context]
                   (swap! *greeting-call-counter inc)
                   (str "Hello, " (context/sub-ctx context username) "!"))]
    (facts
      (context/sub-ctx context-1 greeting) => "Hello, vlaaad!"
    ...
    (let [context-2 (context/swap context-1 assoc-in [:db :user :age] 28)]
      (facts
        ...
        (context/sub-ctx context-2 greeting) => "Hello, vlaaad!"
        (context/sub-ctx context-2 username) => "vlaaad")
      (fact
  ...
  (let [*template-call-counter (atom 0)
        context-1 (context/create {:db {:lang :en
                                        :templates {:en "Hello, %s!"
                                                    :ru "Привет, %s!"}}}
                                  identity)
        lang #(:lang (context/sub-val % :db))
        en-template #(-> % (context/sub-val :db) :templates :en)
        ru-template #(-> % (context/sub-val :db) :templates :ru)
        template (fn [context]
                   (swap! *template-call-counter inc)
                   (case (context/sub-ctx context lang)
                     :en (context/sub-ctx context en-template)
                     :ru (context/sub-ctx context ru-template)))]
    (facts
      "Template depends on [:lang] and [:en-template] subscriptions"
      (context/sub-ctx context-1 template) => "Hello, %s!"
    ...
    (let [context-2 (context/swap context-1 assoc-in [:db :lang] :ru)]
      (facts
        ...
        (context/sub-ctx context-2 template) => "Привет, %s!"
      ...
      (let [context-3 (context/swap context-2 assoc-in [:db :templates :en] "Hi, %s!")]
        (facts
          ...
          (context/sub-ctx context-3 template) => "Привет, %s!"
          @*template-call-counter => 2)))))
  ...
  (let [*template-counter (atom 0)
        *parent-child-counter (atom 0)
        parent-child (fn [context]
                       ...
                       (when (context/sub-val context :parent)
                         (context/sub-val context :child)))
        template (fn [context]
                   (swap! *template-counter inc)
                   (context/sub-ctx context parent-child))

        context (context/create {} identity)
        _ (facts
            "[template] directly depends on [parent-child]. Both have key deps #{:parent}"
            (context/sub-ctx context template) => nil
            @*template-counter => 1
        ...
        _ (facts
            ...
            (context/sub-ctx context template) => :child
            @*template-counter => 2)
        _ (facts
...
(deftest creating-derived-contexts-inside-subs-add-dependency-on-context-itself
  (let [*sub-context-call-counter (atom 0)
        ...
        sub-context (fn [context]
                      ...
                      (let [str-db (str (context/sub-val context :db))]
                        (context/swap context assoc :str-db str-db)))]
    (context/sub-ctx context sub-context)
    (let [context-2 (context/swap context assoc :other-db 2)]
      (context/sub-ctx context-2 sub-context))
    (fact
...
(deftest double-invalidations-don't-trigger-recalculations
  (let [*str-db-call-counter (atom 0)
        ...
        str-db (fn [context]
                 ...
                 (str (context/sub-val context :db)))
        res-1 (context/sub-ctx context-1 str-db)
        _ (context/swap context-1 assoc :db 2)
        context-3 (context/swap context-1 assoc :db 3)
        res-3 (context/sub-ctx context-3 str-db)]
    (facts
...
(deftest cross-context-dependencies-are-not-tracked
  (let [*sub-call-counter (atom 0)
        ...
        sub (fn [_ other-context]
              ...
              (context/sub-val other-context :db))
        _ (context/sub-ctx context-a-1 sub input-context)
        context-a-2 (context/swap context-a-1 assoc :db :b)
        _ (context/sub-ctx context-a-2 sub input-context)]
    (fact
  ...
  (let [*leaked-context (atom nil)
        *leaking-sub-call-counter (atom 0)
        ...
        leaking-sub (fn [context]
                      ...
                      (inc (context/sub-val context :db)))]
    (fact
      (context/sub-ctx context-1 leaking-sub) => 2)
    (fact
      (context/sub-val @*leaked-context :other-db) => :a)
    (let [context-2 (context/swap context-1 assoc :other-db :b)]
      (fact
        (context/sub-ctx context-2 leaking-sub) => 2)
      (fact
...
(deftest you-can-subscribe-to-functions-directly
  (let [*inc-db-call-counter (atom 0)
        ...
        _ (facts
            ...
            (context/sub-ctx context-2 inc-db) => 3
