...
(deftest test-swapping-handlers
  (binding [lifecycle/*in-progress?* false]
    (let [events (atom [])
          prop-on-text-changed (make-prop-tracking-text-changed-listeners #(swap! events conj %))
          opts {:fx.opt/map-event-handler #(swap! events conj %)}
          c1 (fx/create-component
               {:fx/type :text-field
                :text "text"
                prop-on-text-changed #(swap! events conj [:first %])}
               opts)
          ...
          c2 (fx/advance-component
               ...
               {:fx/type :text-field
                :text "text"
                prop-on-text-changed #(swap! events conj [:second %])}
               opts)
          ...
          c3 (fx/advance-component
               ...
               {:fx/type :text-field
                :text "text"
                prop-on-text-changed {:map-event-handler :first}}
               opts)
          ...
          c4 (fx/advance-component
               ...
               {:fx/type :text-field
                :text "text"
                prop-on-text-changed {:map-event-handler :second}}
               opts)
